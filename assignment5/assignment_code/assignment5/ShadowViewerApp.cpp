...
void ShadowViewerApp::SetupScene() {
  ...
  for (auto group : mesh_data.groups) {
      std::unique_ptr<SceneNode> node = make_unique<SceneNode>();
      
      // Mesh
      RenderingComponent& renderer = node->CreateComponent<RenderingComponent>(vertex_obj);
      renderer.SetDrawRange(group.start_face_index, group.num_indices);
