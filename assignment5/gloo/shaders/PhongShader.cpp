...
void PhongShader::SetTargetNode(const SceneNode& node,
  ...
  auto
      ambient_texture = material_ptr->GetAmbientTexture(),
      diffuse_texture = material_ptr->GetDiffuseTexture(),
      specular_texture = material_ptr->GetDiffuseTexture();
    if (ambient_texture != nullptr) {
        ambient_texture->BindToUnit(0);
        SetUniform("material.ambient_texture", 0);
    }
    if (diffuse_texture != nullptr) {
        diffuse_texture->BindToUnit(1);
        SetUniform("material.diffuse_texture", 1);
    }
    if (specular_texture != nullptr) {
        specular_texture->BindToUnit(2);
        SetUniform("material.specular_texture", 2);
    }
}
...
void PhongShader::SetLightSource(const LightComponent& component) const {
  ...
  // Disable shadows
  SetUniform("shadow_info.enabled", false);
}
...
void PhongShader::SetShadowMapping(
  ...
  // corresponding texture unit.
    SetUniform("shadow_info.enabled", true);
    shadow_texture.BindToUnit(3);
    SetUniform("shadow_map", 3);
    SetUniform("shadow_info.lightNDC", world_to_light_ndc_matrix);
}
}  // namespace GLOO
