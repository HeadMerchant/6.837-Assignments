...
void Renderer::RecursiveRetrieve(const SceneNode& node,
    ...
    for (int i = 0; i < node.GetChildrenCount(); i++) {
        ...
        glm::mat4
            localMat = child.GetTransform().GetLocalToParentMatrix(),
            globalMat = model_matrix * localMat;
        auto renderer = child.GetComponentPtr<RenderingComponent>();
        if (child.IsActive() && renderer != nullptr) {
            std::pair<RenderingComponent*, glm::mat4> childInfo
                = std::make_pair<>(renderer, globalMat);
            info.push_back(childInfo);
        }
        RecursiveRetrieve(child, info, globalMat);
    }
}
